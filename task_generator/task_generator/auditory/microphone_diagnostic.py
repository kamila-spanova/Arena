...
class MicrophoneDiagnostic(Node):
    ...
    def _report(self) -> None:
        ...
        estimates = [
            (label, *gcc_phat(audio[first], audio[second], sample_rate_hz=rate, max_tau_seconds=0.002))
        ...
        lines = [
            ...
            *(
                f"{name.replace('_', ' ').upper()}: {level:.1f} dBFS"
                for name, level in zip(CHANNEL_NAMES, levels, strict=True)
            ),
            "GCC-PHAT:",
