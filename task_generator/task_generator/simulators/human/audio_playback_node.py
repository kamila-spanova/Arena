...
FOOTSTEP_VARIANT_TAGS = frozenset({"default", "walnut_planks"})
...
class HumanSoundPlaybackNode(Node):
    ...
    def _cb_sound_event(self, msg: SoundEvent) -> None:
        ...
        if asset_id == "footstep":
            required_tags = frozenset(
                FOOTSTEP_VARIANT_TAGS.intersection(
                    str(tag) for tag in msg.semantic_tags
                )
        ...
        # self.get_logger().warning(f"playing {asset_id} / {sample.sample_id}: "f"{sample.duration_sec:.3f}s, gain={asset.playback_gain_db:.1f} dB" )
