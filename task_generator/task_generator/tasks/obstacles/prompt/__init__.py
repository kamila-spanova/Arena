...
def declare_schema(node: ROSParamServer, ns: Namespace) -> None:
    ...
    declare_enum(node, ns("generation_mode"), "arena", choices=["arena", "behavior", "behavior_tree", "crowded_behavior_tree"], label="Generation mode", description="Generation mode. Under human:=arena: 'arena' generates waypoint-following agents, 'behavior' additionally generates arena_humansim agent-type behavior definitions. Under human:=hunav: 'behavior_tree' and 'crowded_behavior_tree' generate HuNav behaviour trees.")
