...
def test_bundled_default_pedestrian_resolves() -> None:
    path = HumanIdentifier("arenian").resolve_sync().path

    assert path.name == "arenian"
    assert (path / "arenian.sdf").is_file()
