...
def robot_trajectory_from_tf(
    ...
    base_candidates = [
        (key, sorted(rows, key=lambda row: row["timestamp_ns"]))
        for key, rows in transforms.items()
        if key[1].strip("/").endswith("/base_link")
        and any(not row["static"] for row in rows)
    ]
    ...
    for base_key, base_rows in base_candidates:
        ...
        parents = [
            (key, rows)
            for key, rows in transforms.items()
            if _same_frame(key[0], target)
            and key[1].strip("/") == odom_frame
            and rows
            and all(row["static"] for row in rows)
        ]
        if len(parents) == 1:
    ...
    if len(chains) != 1:
        raise ValueError(
            "robot odometry is absent and TF does not contain exactly one "
            f"{target!r}->odom->base_link trajectory; found "
            f"{[(base, parent) for base, _rows, parent, _tf in chains]}"
        )
    ...
    for record in records:
        if record["continuous"]:
            transitions.setdefault(
                (record["source_id"], record["channel_name"]), []
            ).append(record)
        else:
        ...
        intervals.append(
            {
                **{key: exemplar.get(key) for key in (
                    "event_id", "source_id", "source_agent_id",
                    "source_agent_name", "source_type", "sound_type", "asset_id",
                )},
                "continuous": False,
                "start_time_ns": start_ns,
                "end_time_ns": end_ns,
    ...
    for source_id in sorted({row["source_id"] for row in channel_intervals}):
        ...
        for row in source_rows:
            if merged and row["start_time_ns"] <= merged[-1]["end_time_ns"]:
                merged[-1]["end_time_ns"] = max(
                    merged[-1]["end_time_ns"], row["end_time_ns"]
                )
                merged[-1]["channel_names"] = sorted(
                    set(merged[-1]["channel_names"]) | set(row["channel_names"])
                )
            else:
                merged.append(
                    {
        ...
        output.append(
            {
                **row,
                "start_time_ns": start_ns,
                "end_time_ns": end_ns,
                "start_recording_sample_offset": round(
                    (start_ns - capture_start_ns) * sample_rate / 1_000_000_000
                ),
                "end_recording_sample_offset": round(
                    (end_ns - capture_start_ns) * sample_rate / 1_000_000_000
                ),
                "start_audio_sample_index": first_sample_index
                + round((start_ns - capture_start_ns) * sample_rate / 1_000_000_000),
                "end_audio_sample_index": first_sample_index
                + round((end_ns - capture_start_ns) * sample_rate / 1_000_000_000),
            }
        )
...
def activity_labels(
    intervals: list[dict[str, Any]], start_ns: int, end_ns: int
) -> dict[str, Any]:
    active = [
        row for row in intervals
        if row["start_time_ns"] < end_ns and row["end_time_ns"] > start_ns
    ]
    ...
    motor_active = any(
        row.get("source_type") == "robot" or row.get("sound_type") == "motor"
        for row in active
    )
    ...
    *,
    ...
    for start in range(0, rendered.shape[0], hop):
        ...
        common: dict[str, Any] = {
            ...
            **(
                activity_labels(activity_intervals, timestamp_ns, window_end_ns)
                if activity_intervals is not None
                else {"activity_annotations_available": False}
            ),
        }
...
def export(args: argparse.Namespace) -> Path:
    ...
    if data["odom"] or args.robot_odom_topic:
        ...
        odom_topic, robot_rows, robot_frame_transform = robot_trajectory_from_tf(
            data["transforms"], map_frame
        )
    artifact_prefix = args.artifact_prefix or (indexed_run.group("index") if indexed_run else None)
    prefix = f"{artifact_prefix}_" if artifact_prefix else ""
    ...
    if scenario_source.is_file():
        ...
        if recorded_pedestrian_count != expected_pedestrian_count:
            raise ValueError(
                "recorded pedestrian count does not match scenario.yaml: "
                f"expected {expected_pedestrian_count}, recorded {recorded_pedestrian_count}"
            )
    ...
    activity_intervals = (
        build_rendered_activity_intervals(
            ...
            capture_end_ns=int(raw_summary["first_timestamp_ns"])
            + round(raw.shape[0] * 1_000_000_000 / raw_summary["sample_rate"]),
    ...
    if args.require_activity_annotations and not activity_intervals:
        raise ValueError(
            "recording has no rendered sound activity interval overlapping the "
            "capture; rebuild the microphone array and recorder before generating "
            "the final dataset"
        )
    labels = build_labels(
        rendered,
