...
def verify_remote(
    ...
    for rel, local in inventory.items():
        ...
        if int(info.size) != int(local["size"]):
            die(
                f"remote size mismatch for {remote_path}: "
                f"local={local['size']} remote={info.size}"
            )
        ...
        if lfs:
            ...
            if remote_sha and remote_sha != local["sha256"]:
                die(
                    f"remote SHA-256 mismatch for {remote_path}: "
                    f"local={local['sha256']} remote={remote_sha}"
                )
...
def main() -> int:
    ...
    if not token:
        die(
            f"{args.token_env} is not set. Use a Hugging Face write token, e.g. "
            f"`export {args.token_env}=hf_...` before recording."
        )
    ...
    commit = api.upload_folder(
        ...
        commit_message=(
            f"Add acoustic scenario {args.world_name}/{args.scenario_name} "
            f"#{args.execution_index:04d}"
        ),
    )
